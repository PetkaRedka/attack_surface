...
- `GIT_SUPPORT=true` — репозиторием считается только каталог с маркером
  ...
  единственным репозиторием. Git-поддиректории **внутри** репозитория
  становятся его **модулями** (`modules` в конфиге); у репозитория может
  не быть модулей;
...
`проект → репозитории → модули → исходники`:

- **Репозитории** объявляются в конфиге (JSON `repos` / Threagile
  `technical_assets`);
- **Модули** — git-поддиректории внутри репозитория. В авто-конфиге
  обнаруживаются автоматически (как и репозитории), вручную указываются
  полем `modules` у репозитория (JSON) или подразделом `modules` актива
  (Threagile):
  ...
  technical_assets:
    - id: huy
      type: process
      modules: [pizda, zalupa]
  ```

- **Исходники** — каталоги с кодом внутри репозитория/модуля (в графе
  проекта — `sources` у репозитория, с привязкой к модулю).
